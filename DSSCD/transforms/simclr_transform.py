...
class SimCLRTransform():
    ...
    def __init__(self, size):
        ...
        if self.size == 512 or self.size == 256:  # CMU
            ...
            self.train_transform2 = transforms.Compose([#transforms.RandomHorizontalFlip(),
            ...
            self.train_transform3 = transforms.Compose([  # transforms.RandomHorizontalFlip(),
                                                get_color_distortion(),
                                                # transforms.RandomPerspective(distortion_scale=0.2, p=0.3),
                                                transforms.RandomApply([GaussianBlur([.1, 2.])], p=0.5),
                                                # normalize,
    ...
    def __call__(self, x):
        aug1 = self.train_transform(x)
        aug2 = self.train_transform(x)
        if self.copy_paste:
