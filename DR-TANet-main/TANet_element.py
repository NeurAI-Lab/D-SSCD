...
def resnet50(ssl_path, pretrained=True, sslpretrain= False, **kwargs):
    ...
    if sslpretrain==True:
        # ssl_path = '/data/output/vijaya.ramkumar/sscd/runs2/resnet50_bs_16/Sat_Jun_19_12:50:57_2021/checkpoint_model_500_model1.pth'
        pretrained_dict = torch.load(ssl_path)
        pretrained_dict_new = {}
        for key, value in pretrained_dict.items():
            if 'f.' in key[:2]:
                bbone_name = ".".join(key.split(".")[1:])
                if bbone_name in model.state_dict().keys():
                    pretrained_dict_new[bbone_name] = value
        model.load_state_dict(pretrained_dict_new,strict=True)
        print(' self pretrained loaded', ssl_path)
    if pretrained==True:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']), strict=False)  #
        print('baseline with imagenet pretraind')
    channels = [256, 512, 1024, 2048]
    return model,channels

def get_encoder(arch,ssl_path, pretrained=True, sslpretrain=False):
    if arch == 'resnet18':
        return resnet18(pretrained)
    elif arch == 'resnet34':
        return resnet34(pretrained)
    elif arch == 'resnet50':
        return resnet50(ssl_path,pretrained, sslpretrain)
    else:
