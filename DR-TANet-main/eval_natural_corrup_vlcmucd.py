...
class Evaluate:
    ...
    def store_imgs_and_cal_matrics(self, t0, t1, mask_gt, mask_pred, type,severity):
        ...
        dir_img2 = pjoin(self.dir_img,type,severity)
        self.fn_img = pjoin(dir_img2, '{0}-{1:08d}.png'.format(self.ds, self.index))

        fn_save = self.fn_img

        if not os.path.exists(dir_img2):
            os.makedirs(dir_img2)
    ...
    def run(self):
        ...
        # state_dic = {k.partition('module.')[2]:v for k,v in torch.load(self.fn_model).items()}
        if self.args.multi_gpu:
            self.model = nn.DataParallel(self.model)
        self.model.load_state_dict((torch.load(self.fn_model)))  #(torch.load(self.fn_model))
...
class evaluate_pcd(Evaluate):
    ...
    def run(self, set):
        ...
        self.fn_model = pjoin(self.args.checkpointdir, 'set{0:1d}'.format(self.set), 'checkpointdir', '00035100.pth')
...
class evaluate_cmu(Evaluate):
    ...
    def Init(self):
        ...
        self.fn_model = pjoin(self.args.checkpointdir, 'checkpointdir', '00070050.pth')  #00070050  00035100

    def eval(self, type,severity):
        ...
        mask_gt = np.squeeze(np.where(self.mask==True,255,0),axis=0)
        if self.args.store_imgs:
            precision, recall, accuracy, f1_score = self.store_imgs_and_cal_matrics(img_t0,img_t1,mask_gt,mask_pred,type,severity)
        else:
            precision, recall, accuracy, f1_score = self.cal_metrcis(mask_pred,mask_gt)
        return (precision, recall, accuracy, f1_score)

    def run(self, type,severity):
        super(evaluate_cmu, self).run()


        testdir = [0,6,7,9,12,23,24,25,27,28,32,34,36,38,39,45,47,48,50,56,58,60,61,64,66,69,76,77,81,82,85,92,93,94,95,97,100,106,107,112,113,117,119,120,125,129,132,134,135,139,142,144,145,150]
        img_cnt = 0
        metrics = np.array([0, 0, 0, 0], dtype='float64')
        for idx in testdir:
            test_loader = datasets.vl_cmu_cd_eval(pjoin(self.args.datadir,type,severity, 'raw', '{:03d}'.format(idx)))
            img_cnt += len(test_loader)
            self.ds = idx
            for i in range(0, len(test_loader)):
                ...
                metrics += np.array(self.eval(type,severity))
        precision = (metrics[0] / img_cnt)
        recall = (metrics[1] / img_cnt)
        accuracy = (metrics[2] / img_cnt)
        fscore = (metrics[3] / img_cnt)
...
if __name__ =='__main__':
    ...
    if parser.parse_args().dataset == 'pcd':
        ...
        eval.Init()
        types = ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur',
                         'glass_blur', 'motion_blur', 'zoom_blur', 'snow',  'fog',
                         'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']
        severity = ['1','2','3','4','5']
        resultdir = args.resultdir
        f_metrics = open(pjoin(resultdir, "eval_metrics(dataset_naturalcorrup).csv"), 'a+')
        metrics_writer = csv.writer(f_metrics)
        metrics_writer.writerow(['corrup_type', 'severity', 'precision', 'recall','accuracy','fscore'])
        for type in types:
            for num in severity:
                precision, recall,accuracy, fscore = eval.run(type,num)
                metrics_writer.writerow([type, num,'%.3f' % precision, '%.3f' % recall,
                                         '%.3f' % accuracy, '%.3f' % fscore])
